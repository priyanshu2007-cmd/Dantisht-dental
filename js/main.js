document.addEventListener("DOMContentLoaded", () => {
  ...
  if (tw) {
    const text = tw.dataset.text || tw.innerText;
    tw.innerText = "";
    let i = 0;
    function type() {
      if (i < text.length) {
        ...
        i++;
        setTimeout(type, 80);
      } else tw.classList.remove("typewriter-cursor");
    }
    setTimeout(() => {
  ...
  if (typeof gsap !== "undefined" && typeof ScrollTrigger !== "undefined") {
      ...
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            ...
            e.target.style.transition = ".6s ease";
    ...
    document
      .querySelectorAll(".gsap-fade-up,.gsap-stagger-item")
  ...
  document.querySelectorAll(".acc-header").forEach((h) => {
    h.addEventListener("click", () => {
      const c = h.nextElementSibling,
        icon = h.querySelector("i");
      c.classList.toggle("active");
      if (icon)
        (icon.classList.toggle("fa-plus"), icon.classList.toggle("fa-minus"));
    });
  });
});
