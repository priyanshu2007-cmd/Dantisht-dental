document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".ba-card").forEach((card) => {
    const slider = card.querySelector(".ba-slider"),
      after = card.querySelector(".ba-after"),
      handle = card.querySelector(".ba-handle");
    ...
    const move = (e) => {
      ...
      let x =
        (e.type.includes("touch") ? e.touches[0].clientX : e.clientX) - r.left;
  ...
  carousel.addEventListener("mousemove", (e) => {
    ...
    e.preventDefault();
    carousel.scrollLeft =
      scrollL - (e.pageX - carousel.offsetLeft - startX) * 2;
  });
